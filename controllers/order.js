...
exports.getOrder = async (req, res) => {
  ...
  for (let order of orders) {
    ...
    const orderItems = await OrderItem.findAll({ where: { order_id: orderId }, include: [{ model: Product }] });
...
exports.createOrder = async (req, res) => {
  ...
  const order = await Order.create({ user_id: userId, total_price: totalPrice });
  const orderId = order.order_id;
  for (let orderItem of orderItems) {
    await OrderItem.create({ order_id: orderId, product_id: orderItem.productId, quantity: orderItem.quantity, price: orderItem.price });
  }
